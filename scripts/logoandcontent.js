...
console.log("🚑 FIXING: Logo Size & Empty Sidebars...");

// =============================================================================
// 1. FIX DOCFX.JSON (CRITICAL: Enable Sidebars Globally)
// =============================================================================
if (fs.existsSync(DOCFX_JSON_PATH)) {
    try {
        ...
        // Ensure Metadata allows sidebars
        if (!config.build.globalMetadata) config.build.globalMetadata = {};
        
        // DELETE ANY DISABLE FLAGS (This is why sidebars were empty)
        ...
        delete config.build.globalMetadata._disableContributors;

        // Force Markdig for HTML rendering
        config.build.markdownEngineName = "markdig";
        config.build.markdownEngineProperties = { "html": true };

        fs.writeFileSync(DOCFX_JSON_PATH, JSON.stringify(config, null, 2));
        console.log("✅ docfx.json updated: Sidebars globally enabled.");
    } catch (e) {
...
// =============================================================================
// 2. OVERWRITE MAIN.CSS (Bigger Logo + Visible Text)
// =============================================================================
const cssContent = `
/* ==========================================================================
   MICROSOFT .NET STYLE SYSTEM (FINAL POLISH)
   ========================================================================== */
:root {
  --net-purple: #512bd4;
...
.navbar-brand img, img#logo {
    height: 40px !important; /* BIGGER (Was 24px) */
...
/* HIDE EDIT BUTTON */
.contribution-link { display: none !important; }
...
console.log("✅ main.css updated: Logo resized to 40px, Sidebars forced visible.");
